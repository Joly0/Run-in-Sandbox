...
# Prompts user for deep-clean consent during interactive installs
function Get-DeepCleanConsent {
    param([switch]$DeepCleanRef)
...
# Performs deep-clean of registry entries if requested
...
# Downloads and extracts the package archive from GitHub
...
# Gets the default startup script names from the extracted package
...
# Backs up custom startup scripts that are not part of the default installation
...
# Removes old installation if deep-clean is requested
...
# Restores custom startup scripts after installation
...
# Updates core files from the extracted package to the installation folder
function Update-CoreFiles {
    ...
    if (Test-Path $sourceSandboxDir) {
        ...
        # Handle Sandbox_Config.xml
        $sourceConfig = Join-Path $sourceSandboxDir "Sandbox_Config.xml"
        $destConfig = Join-Path $destSandboxDir "Sandbox_Config.xml"
        if (Test-Path $sourceConfig) {
            if ($IsInstalled -and (Test-Path $destConfig)) {
                # For updates: backup existing config, copy new one, then merge user settings
                $configBackup = "$env:TEMP\Sandbox_Config_Update_Backup.xml"
                Copy-Item $destConfig $configBackup -Force
                Write-Verbose "Backed up existing Sandbox_Config.xml to $configBackup"
                Copy-Item $sourceConfig $destConfig -Force
                Write-Verbose "Copied new Sandbox_Config.xml, will merge user settings"
                
                # Merge user settings from backup into new config
                if (Get-Command Merge-SandboxConfig -ErrorAction SilentlyContinue) {
                    try {
                        Merge-SandboxConfig -OldConfigPath $configBackup -NewConfigPath $destConfig
                        Write-Verbose "User settings merged into new config successfully"
                    } catch {
                        Write-Verbose "Config merge failed: $($_.Exception.Message)"
                        # If merge fails, restore the backup to preserve user settings
                        Copy-Item $configBackup $destConfig -Force
                        Write-Verbose "Restored original config due to merge failure"
                    }
                } else {
                    Write-Verbose "Merge-SandboxConfig not available, restoring user config"
                    # If merge function not available, restore user's config to preserve their settings
                    Copy-Item $configBackup $destConfig -Force
                }
                Remove-Item $configBackup -Force -ErrorAction SilentlyContinue
            } else {
                # For fresh installs: just copy the config
                Copy-Item $sourceConfig $destConfig -Force
                Write-Verbose "Copied Sandbox_Config.xml for fresh installation"
            }
        }
        ...
        if (Test-Path $defaultStartupDir) {
            ...
            # For updates: update default scripts, preserve custom ones
            # For fresh installs: copy all scripts
            Get-ChildItem -Path $defaultStartupDir -File | ForEach-Object {
                $destScript = Join-Path $destStartupDir $_.Name
                if ($IsInstalled) {
                    # For updates: always update default scripts (they may have bug fixes)
                    if ($DefaultNames -contains $_.Name) {
                        Copy-Item $_.FullName $destScript -Force
                        Write-Verbose "Updated default startup script: $($_.Name)"
                    } elseif (-not (Test-Path $destScript)) {
                        # New default script added in this version
                        Copy-Item $_.FullName $destScript -Force
                        Write-Verbose "Added new startup script: $($_.Name)"
                    }
                } else {
...
# Invokes the Add_Structure.ps1 script to set up context menu entries
function Invoke-AddStructure {
    ...
    Get-ChildItem -LiteralPath $ExtractPath -Recurse -Filter "*.psm1" | Unblock-File
