...
function Add-RegItem {
    ...
    try {
        ...
        if ($Is_Entry_Complete) {
            Write-LogDetail -Message "Registry key already exists and is complete: $Key_Label_Path"
            Write-LogMessage -Message_Type "SUCCESS" -Message "Context menu for $Info_Type has already been added and is complete"
        ...
        if (Test-Path -Path $Key_Label_Path) {
            ...
            if ($Is_Entry_Complete_After_Repair) {
                Write-LogDetail -Message "Registry key repaired successfully: $Key_Label_Path"
                Write-LogMessage -Message_Type "SUCCESS" -Message "Context menu for $Info_Type has been successfully repaired"
                Add-Content -Path $RegistryPathsFile -Value $Key_Label_Path
                return
            } else {
                Write-LogDetail -Message "Registry key repair incomplete, recreating: $Key_Label_Path"
                Write-LogMessage -Message_Type "WARNING" -Message "Context menu for $Info_Type could not be fully repaired, proceeding with full recreation"
            }
        }
...
function Find-RegistryIconPaths {
    ...
    $currentPath = $null
    # Regex patterns for parsing registry export file
    $keyPattern = '^\[([^\]]+)\]$'
    $iconPattern = '^\s*"Icon"="([^"]+)"$'
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match $iconPattern -and $matches[1] -eq $iconValueToMatch) {
